...
def get_mae_metrics(path):
    'change opt parameters to False in order to run without hyperparameters optimization'
    mae = []
    ...
    Y_pred_simple = esn_simple.get_predictions(opt=True)
    ...
    Y_pred_deep = esn_deep.get_predictions(opt=True)
    ...
    Y_pred_hier = esn_hier.get_predictions(opt=True)
    ...
    Y_pred_gru = gru.get_predictions(opt=True)
